...
- `planning_agent.service.run_planning_agent` 只执行完整 A/B/C 链路。
...
Python 调用方不需要修改 C 的请求或响应字段，但 Dify 管理员需要重新导入并发布 `dify/Research Planning Agent.yml`。快速版 C 从四个串行 LLM 节点缩减为一个，并关闭 thinking；C 强制使用 B 的选中设计，缺少 `selected_design` 时按契约错误失败。
...
- 正式 Planning 执行已统一为 A/B/C runner，后续只需扩展进度持久化和前端展示。
